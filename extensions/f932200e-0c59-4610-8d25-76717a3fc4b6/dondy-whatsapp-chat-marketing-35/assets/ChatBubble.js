...
(() => {
  ...
  function shouldShowWidgetOnCurrentPage(showOnThesePages) {
  ...
  const showWidgetBasedOnCurrentPage = (hasShowOnThesePages, showOnThesePages) => {
    const showTheWidget = shouldShowWidgetOnCurrentPage(showOnThesePages)
  ...
  const showWidgetBasedOnSelectedDevice = (showWidgetOnSelectedDevice) => {
    const deviceType = showWidgetOnSelectedDevice;  // The value of this field can be `Both`, `Phone`, or `Desktop`
    const isMobile = mobileCheck() || detectMob();
  ...
  const BASE_URL = "https://chat-widget-view-service-396524864226.us-east4.run.app/api";
  ...
  const changeHtmlAccordingToChatWidgetDetails = async () => {
    const chatWidgetDetails = await fetch(
      `${BASE_URL}/WhatsAppWidgetsView/${window.Shopify?.shop}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!showWidgetBasedOnCurrentPage(chatWidgetDetails?.hasShowOnThesePages, chatWidgetDetails?.showOnThesePages)) {
      chatBubbleElement?.classList.remove("whatsapp-widget-visible");
      return;
    }

    if (!showWidgetBasedOnSelectedDevice(chatWidgetDetails?.showWidgetOnSelectedDevice)) {
      chatBubbleElement?.classList.remove("whatsapp-widget-visible");
      return;
    }
  };
